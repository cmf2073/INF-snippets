...
for f in *.csv.FIN
	do
		echo 'Importando: ['"$f"']'
		mysql -h$MySQL_host -u$MySQL_USR -p$MySQL_ID -P$MySQL_port -e "use DBclaro;LOAD DATA LOCAL INFILE '/Users/pulman/Documents/pablo.ulman/Documentos/Paquete-Ideas/proceso/amdocs/"$CARPETAINPUT"/"$f"' IGNORE INTO TABLE DBclaro.$TABLA FIELDS TERMINATED BY '|' LINES TERMINATED BY '\n' (addd, texto, phoneNumber, status,sms);"
		echo 
	done
echo '--------------------------------------'
...
echo 'Se generarion los Archivos:'
echo "/tmp/$ARCHIVO1.csv"
echo "/tmp/$ARCHIVO2.csv"
#echo "/tmp/$ARCHIVO1-$TIMENOW.csv"
#echo "/tmp/$ARCHIVO2-$TIMENOW.csv"
